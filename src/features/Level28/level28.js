...
export default Level28 = (props) => {
    console.disableYellowBox = true; 

    const [answer, setAnswer] = useState(null)
    const [loading, setLoading] = useState(true)
    const [question] = useState(props.route.params.question)
    ...
    const addNumber = (number) => {
        ...
        if( answer != null && answer.length > 6) {
            const nextProblem = generateRandomNumber()
            if(question == `${answer}${number}`) {
                props.navigation.navigate('Level29', { 
            ...
            <View>
                ...
                <View>
                    ...
                    <View style={styles.inputView2}>
                        <View style={styles.textInputView}>
                            <Text style={styles.textNumber}>{ question.toString()[5] }</Text>
                        </View>
                        <View style={styles.textInputView}>
                            <Text style={styles.textNumber}>{ question.toString()[6] }</Text>  
                        </View>
                        <View style={styles.textInputView}>
                            <Text style={styles.textNumber}>{ question.toString()[7] }</Text>  
                        </View>
                    </View>
    ...
    const renderAnswerDeck = () => {
        return(
            <View>
                ...
                <View style={styles.inputView2}>
                    <View style={styles.textInputView}>
                        <Text style={styles.textNumber}>{ answer == null ? '' : answer.toString()[5] }</Text>
                    </View>
                    <View style={styles.textInputView}>
                        <Text style={styles.textNumber}>{ answer == null ? '' : answer.toString()[6] }</Text>  
                    </View>
                    <View style={styles.textInputView}>
                        <Text style={styles.textNumber}>{ answer == null ? '' : answer.toString()[7] }</Text>  
                    </View>
                </View>
            </View>
        )
    ...
    return (
        <View style={styles.mainView}>
            ...
            <AppHeader title="NumberGame"></AppHeader>
            <View style={styles.scoreAndHeading}>
                <View>
                    <Text style={styles.heading1}>Level 28</Text>
                    <Text style={styles.difficulty1}>Difficulty Level: Hard</Text>
                </View>
